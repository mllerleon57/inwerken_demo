	...
	return Controller.extend("sap.ui.demo.basicTemplate.controller.Game", {
		...
		onInit: function () {
			...
			this.getOwnerComponent().attachWsCallback((msg)=>{
				...
				let modalOn = this.getView().getModel("viewModel").getProperty('/modalOn')
				let kanbanMode = this.getView().getModel("viewModel").getProperty('/kanbanMode')
			...
			oRouter.getRoute("game").attachPatternMatched(this._onObjectMatched, this);   
		...
		onButtonPress:function(){
			...
			let sProperty = this.getView().byId("mainGameTitle").getProperty("title")				
			...
			if(this._handleGameMode()) {
				...
				oView.getModel().setProperty(`/lists/${iIndex}/solved`,true)
			...
			if(oView.getModel("viewModel").getProperty("/modalOn")===true) return false
			//open dialog
			
			let oDialog = oView.getModel("viewModel").getProperty("/kanbanMode") === true ? this._openKanabanDialog() : this._openStandardDialog()
		...
		_openStandardDialog(){
			...
			if (!oDialog) {
				...
				oDialog = sap.ui.xmlfragment(this.getView().getId(), "sap.ui.demo.basicTemplate.view.ResultDialog",this);
		...
		_openKanabanDialog(){
			...
			if (!oDialog) {
				// create dialog via fragment factory
				oDialog = sap.ui.xmlfragment(this.getView().getId(), "sap.ui.demo.basicTemplate.view.KanbanResultDialog",this);
				this.getView().addDependent(oDialog);
			}
			return oDialog
		},
		...
		_onObjectMatched: function (oEvent) {
			...
			this.getView().byId("lb1-listUl").bindElement("/lists/"+ path)

			this.getView().byId("mainGameTitle").bindElement("/lists/"+ path)
			this.getView().byId("objectHeader1").bindElement("/lists/"+ path)

			let gameMode = this.getView().getModel().getProperty(`/lists/${path}/gameMode`)
			this.formatCode = this.getView().getModel().getProperty(`/lists/${path}/formatCode`)
		...
		onSwitchChange:function(oEvent){
			...
			console.log(this.getView().getModel('viewModel').getProperty('/kanbanMode'))
		},
		...
		_areBoxesInRightOrder:function(){
			...
			const sortedItems = [...document.querySelectorAll('.list2 .list-item')].map(el => el.textContent)
		...
		_areCorrectBoxesSelected:function(){
			let aData = this.getView().byId("lb1-listUl").getBinding("items").oList
			const oModel = this.getOwnerComponent().getModel();
			const selectedItems = [...document.querySelectorAll('.list2 .list-item')].map(el => el.textContent)
			
			let trueValues = aData.filter(el => el.correct === true).map(el => el.name)
			...
			return resultArray.length === trueValues.length && trueValues.length === selectedItems.length
		},
		...
		_handleListSorting:function(){		
			...
			// random sort
			//compare 
			let randomizedArray = this._shuffleArray(oList.map((el,ind)=>el.name.toUpperCase()))

			let myComparator = (a,b) => {             

                return randomizedArray.indexOf(a) > randomizedArray.indexOf(b) ? 1 : -1
            }
		...
		_shuffleArray:function(array) {
		  ...
		  _isYearInputCorrect:function() {
			...
			let result = !year ? false : ((year % 4 == 0) && (year % 100 != 0)) || (year % 400 == 0); 
			return result;
		  },
		  _openWrongYearModal:function() {
			...
			if (!oDialog) {
				// create dialog via fragment factory
				oDialog = sap.ui.xmlfragment(this.getView().getId(), "sap.ui.demo.basicTemplate.view.WrongYearDialog",this);
				this.getView().addDependent(oDialog);
			}
